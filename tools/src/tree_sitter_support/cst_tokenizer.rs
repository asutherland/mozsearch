...
/// Helper to identify the conceptual namespace for the semantic contexts
/// generated by `hypertokenize_source_file`.  Currently these are just the
/// languages determined internally by that function, but it could make sense in
/// the future for us to potentially map multiple distinct languages to a
/// smaller namespace.  For example, if we supported both MarkDown and rST in a
/// first-class fashion via tree-sitter, it could make sense to put them both in
/// the same namespace.
pub fn namespace_for_file(path: &Path) -> &'static str {
    ...
    // Note that this needs to be updated when `hypertokenize_source_file` is
...
pub fn hypertokenize_source_file(
    ...
    //
    // Note that it's necessary to update `namespace_for_file` helper above
    // when updating this mapping.
